class Api::V4::CommentsController < Api::V4::ApiController
  ...
  def index
    ...
    response_comments = comments.as_api_response(:basic)

    # Mark comments as read or not if feedback exists
    comments.each do |c|
      read = false
      feedback = current_user.feedbacks.where(stable: @task.stable, notifiable: c).first # pretty sure then can only be one feedback per comment, right?
      unless feedback.nil?
        read = feedback.read_feedbacks.where(user_id: current_user.id).count() != 0
        puts read
      end
      response_comments.detect{ |r| r[:id] == c.id}[:read] = read
    end

    users = user_index(@task.stable, params[:user_since])

    render json: {comments: response_comments, users: users}
  end
