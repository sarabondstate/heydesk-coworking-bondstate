class Api::V4::TagsController < Api::V4::ApiController
  ...
  def index
    tags = @stable.tags
    if @stable.stable_type_id == 2
      tags << Tag.where(tag_type_id: 5).limit(1)
    end
  ...
  def horse_tags
    HorsesTags.get_all_for_horse(@horse)

    ht_arr = HorsesTags.find_by_sql(["
        SELECT horses_tags.* FROM horses_tags
        LEFT JOIN tags ON tags.id = horses_tags.tag_id
        LEFT JOIN tags_tasks ON tags_tasks.tag_id = tags.id
        LEFT JOIN tasks ON tasks.id = tags_tasks.task_id
        WHERE horses_tags.tag_updated IS NOT NULL
        AND tasks.horse_id = ?
        ORDER BY horses_tags.tag_updated DESC",
        @horse.id]
    )

    ht = HorsesTags.where(id: ht_arr.map(&:id))
    ht = limit_tags(ht, @horse.stable)
  ...
  def show
    objects = HorsesTags.get_newest_relations(@horse, Tag.find(params[:id]), params[:limit].try(:to_i) || 50)
