class Api::V1::ActivitiesController < Api::V1::ApiController
  ...
  def index
    ...
    tasks = all_tasks_for_horse.where.not(completed_at: nil).order(completed_at: :desc).limit(limit).map do |task|
  ...
  def my_activities
    ...
    tasks = Task.where(stable: stable.id, completed_by_id: current_user.id).order(completed_at: :desc).limit(limit).map do |task|
      task.as_hash_with_extra_info(current_user)
    end
    comments = current_user.comments.where(task_id: Task.where(stable: stable.id)).order(created_at: :desc).limit(limit).to_a
