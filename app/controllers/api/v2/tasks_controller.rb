class Api::V2::TasksController < Api::V2::ApiController
    ...
    ActiveRecord::Base.transaction do
      params[:task][:horse_dates].each do |horse_data|
        ...
        if horse_data[:completed] === 'true'
          tp[:completed_at] = horse_data[:date]
  ...
  private
  def task_params
    params.require(:task).permit(:title, :date, :type_id, :time, :note, :horse_id, tag_ids: [], custom_field_values_attributes:[:id, :custom_field_id, :value_one, :value_two, :_destroy])
  end
end
